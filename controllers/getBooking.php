			...
			foreach($segmentVal as $segmentSubKey=>$segmentSubVal){
				...
				if(isset($segmentSubVal['CabinBaggage']) && !empty($segmentSubVal['CabinBaggage'])){
					...
					if($segmentSubVal['CabinBaggage'] > 0){
						$_AcabinAllowance['Value'] = $segmentSubVal['CabinBaggage'];
						$_AcabinAllowance['Unit']  = 'Kilograms';
					}
				}
				...
				if(count($_AbaggageAllowance) > 0){
					$viaFlights[$segmentSubKey]['cabinbaggageAllowance'] = $_AcabinAllowance;
				}
...
/* 		$_IapiPublishedFare		= $_AfareInfo['PublishedFare'];
		...
		if($_SapiCurrency == $_SuserCurrency){
			...
			//$_IapiTax				= $_IapiTax+$_IapiTdsOnCommission;
			//$_IapiTotalFare		= $_IapiTotalFare+$_IapiTdsOnCommission;
		...
		foreach($_Apassengers as $paxKey=>$paxVal){
			...
			//echo "<pre>";print_r($paxVal['Baggage']);die;
			...
			if(isset($_Asegments) && !empty($_Asegments)){
				$_AmealSeatTempArr = array();
				foreach($_Asegments as $mealBaggageTempKey=>$mealBaggageTempVal){
					
					if(isset($mealBaggageTempVal['Baggage'])){
					
						$_AbaggageArr[0] = $mealBaggageTempVal['Baggage'];
						$_AbaggageArr[1] = $mealBaggageTempVal['CabinBaggage'];
						//echo "<pre>";print_r($_AbaggageArr);
						...
						if($_AbaggageArr[0]!='0 Kg')
						{
							...
							$_AmealSeatTempArr['ssrtype']	= 'Baggage';
							$_AmealSeatTempArr['ssrtype1']	= 'Free Baggage';

							$_Atemp['ssr'][] = $_AmealSeatTempArr;
						}
						if(isset($_AbaggageArr[1])){
							
							if($_AbaggageArr[1]!='0 Kg')
							{
								$_IapiPrice		= 0;
								...
								$_AmealSeatTempArr['carrierCode']	= $_Asegments[$mealBaggageTempKey]['Airline']['AirlineCode'];
								$_AmealSeatTempArr['flightNumber']	= $_Asegments[$mealBaggageTempKey]['Airline']['FlightNumber'];
								$_AmealSeatTempArr['code']			= $_AbaggageArr[1];

								$_AmealSeatTempArr['description']	= $_AbaggageArr[1].' Free Cabin Baggage';

								$_AmealSeatTempArr['Text'] = $_AbaggageArr[1].' Free Cabin Baggage'; 
								$_AmealSeatTempArr['amount'] = $_IapiPrice;
								
								$_AmealSeatTempArr['ssrtype']	= 'Baggage';
								$_AmealSeatTempArr['ssrtype1']	= 'Cabin Baggage';
		...
		/* TDS commission enabled*/
		$_IapiTax				= $_IapiTax+$_IapiTdsOnCommission;
		$_IapiTotalFare			= $_IapiTotalFare+$_IapiTdsOnCommission;
		
		$_IdefaultTax			= $_IdefaultTax+$_IdefaultTdsOnCommission;
		$_IdefaultTotalFare		= $_IdefaultTotalFare+$_IdefaultTdsOnCommission;
		
		$_IuserTax				= $_IuserTax+$_IuserTdsOnCommission;
		$_IuserTotalFare		= $_IuserTotalFare+$_IuserTdsOnCommission;
		...
		foreach($_AtripFlights as $tripKey=>&$tripVal){
			...
			if($tripKey == 0){
				...
				$_AreturnData['PLBEarned']   		= $_IuserPLBEarned;
				$_AreturnData['IncentiveEarned']   	= $_IuserIncentiveEarned;
				$_AreturnData['TdsOnCommission']   	= $_IuserTdsOnCommission;
				$_AreturnData['TdsOnPLB']   		= $_IuserTdsOnPLB;
				$_AreturnData['TdsOnIncentive']   	= $_IuserTdsOnIncentive;
