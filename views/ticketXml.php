<?php
/**
	@File Name 		:	ticketXml.php
	@Author 		:	Ramanathan 
	@Created Date	:	2016-10-20 10:55 AM
	@Description	:	ticketXml input
*/

function ticketXml($thisObj)
{
	$_Ainput 				= array();
	...
	if(isset($thisObj->_Ainput['flightResultIndex'])){
	...
	if(isset($thisObj->_Ainput['inputPassengers']) && count($thisObj->_Ainput['inputPassengers']) > 0){
		...
		foreach($thisObj->_Ainput['inputPassengers'] as $paxKey=>$paxVal){
			...
			if($_IisLead == 'false' && $_IpaxType == 1 && $paxKey==0){
				$_IisLead = 'true';
			}
			...
			if($paxVal['dob']=='0000-00-00T00:00:00'){
			...
			$_Ainput['Passengers'][$paxKey]['Nationality'] 			= $paxVal['nationality'];
	...
	if(isset($thisObj->_Ainput['pnr']) && !empty($thisObj->_Ainput['pnr'])){
		...
		if(isset($_Ainput['ResultIndex'])){
			unset($_Ainput['ResultIndex']);
		}
		
		if(isset($_Ainput['TraceId'])){
			unset($_Ainput['TraceId']);
		}
	}
